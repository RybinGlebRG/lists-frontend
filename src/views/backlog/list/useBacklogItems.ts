...
import { setReloaded, reload, setInitialized, setBacklogItems } from '../backlogSlice'
...
export default function useBacklogItems(){
    ...
    let createBacklogItem = (backlogItemCreateReqauest: BacklogItemCreateRequest, onUpdate: () => void) => {
        BacklogRepository.create(backlogItemCreateReqauest, store.userId, store.JWT, ()=> dispatch(openSignIn(null)))
        .then(result => {
            setUpdateError(null);
            setIsUpdated(true);
            dispatch(reload());
    ...
    let deleteBacklogItem = (backlogItemId: number) => {
        BacklogRepository.deleteBacklogItem(backlogItemId, store.userId, store.JWT, ()=> dispatch(openSignIn(null)))
        .then(result => {
            setDeleteError(null);
            setIsDeleted(true);
            dispatch(reload());
